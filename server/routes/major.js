...
router.get("/list", (req, res) => {
    ...
    major.lesson_list,
    major.remark,
    (SELECT SUM(lesson.length) FROM lesson WHERE INSTR(CONCAT(",",major.lesson_list,","),CONCAT(",",lesson.lesson_id,","))) AS total
    FROM
    major
    ORDER BY major_id
    `;
...
router.post("/create", (req, res) => {
    ...
    const $sql = `
        INSERT INTO major(major_name, lesson_list, remark)
        VALUES(?,?,?)
    `;
